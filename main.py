...
if __name__ == '__main__':
    ...
    parser.add_argument('--similarity_type', default='dot', type=str, choices=['dot', 'cosine', 'tonimoto', 'pearson'],
                        help='similarity type, it only works for k-means capsule classifier')
    ...
    CLASSIFIER_TYPE, SIMILARITY_TYPE, EMBEDDING_SIZE = opt.classifier_type, opt.similarity_type, opt.embedding_size
    ...
    model = Model(VOCAB_SIZE, EMBEDDING_SIZE, NUM_CODEBOOK, NUM_CODEWORD, HIDDEN_SIZE, IN_LENGTH, OUT_LENGTH, NUM_CLASS,
                  ROUTING_TYPE, EMBEDDING_TYPE, CLASSIFIER_TYPE, SIMILARITY_TYPE, NUM_ITERATIONS)
